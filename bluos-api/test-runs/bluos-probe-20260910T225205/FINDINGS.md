...
| | |
|---|---|
| bundle | `bluos-probe-20260910T225205` |
| date | 2026-09-10 |
...
|---|---|---|---|---|---|
| `C-48-setmaster-self` | **DISCONFIRMED** | /SetMaster?master=<own address> is handled gracefully | untested | 5.3 | `029-state_setmaster` |
| `C-49-setmaster-badtarget` | **DISCONFIRMED** | /SetMaster?master=<not a player> is handled gracefully | untested | 5.3 | `035-state_setmaster` |
| `C-46-setmaster-stale-response` | **MIXED** | /SetMaster answers with the PRE-call SyncStatus, same etag | hardware, single pass | 5.3 | `002-state_setmaster`, `006-state_setmaster`, `012-state_setmaster`, `015-state_setmaster`, `022-state_setmaster`, `028-state_setmaster` |
| `C-39-master-swap` | **CONFIRMED** | master and slave roles can be swapped without ungrouping first | open question raised by the tester | 5.3 | `049-state_setmaster` |
| `C-41-setmaster-bare-standalone` | **CONFIRMED** | a bare /SetMaster on a standalone player is a no-op | hardware, single pass | 5.3 | `003-state_setmaster` |
| `C-42-setmaster-bare-master` | **CONFIRMED** | a bare /SetMaster on a master is a no-op and does not dissolve its group | hardware, single pass | 5.3 | `007-state_setmaster` |
| `C-43-setmaster-bare-slave` | **CONFIRMED** | a bare /SetMaster on a slave leaves the group | hardware, single pass | 5.3 | `013-state_setmaster` |
| `C-44-setmaster-master-param` | **CONFIRMED** | /SetMaster?master= joins that player's group | hardware, single pass | 5.3 | `016-state_setmaster` |
| `C-45-setmaster-slave-param` | **CONFIRMED** | /SetMaster ignores a slave= parameter | hardware, single pass | 5.3 | `057-state_setmaster` |
| `C-47-setmaster-noport` | **CONFIRMED** | port is optional on /SetMaster?master= | untested | 5.3 | `023-state_setmaster` |
| `C-50-setmaster-reparent` | **CONFIRMED** | a slave can be reparented onto another master with ?master= | bluos-dashboard (orphan recovery) | 5.1 | `042-state_setmaster` |
| `C-51-setmaster-nested` | **CONFIRMED** | a bare /SetMaster on a nested master detaches it cleanly | untested | 14 | `067-state_setmaster` |
| `C-54-setmaster-one-sided` | **CONFIRMED** | a group formed by /SetMaster?master= is one-sided: the master does not list the joiner as a <slave> | hardware, this project | 5.3 | `017-state_setmaster` |
...
```markdown
| `C-46-setmaster-stale-response` | /SetMaster answers with the PRE-call SyncStatus, same etag | hardware, single pass | MIXED | 2026-09-10, fw 4.16.22, schema 34 | `- :0(analysis)` ->  | bluos-probe-20260910T225205: 002-state_setmaster, 006-state_setmaster, 012-state_setmaster, 015-state_setmaster |
| `C-48-setmaster-self` | /SetMaster?master=<own address> is handled gracefully | untested | DISCONFIRMED | 2026-09-10, fw 4.16.22, schema 34 | `- :0(analysis)` ->  | bluos-probe-20260910T225205: 029-state_setmaster |
| `C-49-setmaster-badtarget` | /SetMaster?master=<not a player> is handled gracefully | untested | DISCONFIRMED | 2026-09-10, fw 4.16.22, schema 34 | `- :0(analysis)` ->  | bluos-probe-20260910T225205: 035-state_setmaster |
```
...
- `C-39-master-swap` -- master and slave roles can be swapped without ungrouping first (section 5.3)
- `C-41-setmaster-bare-standalone` -- a bare /SetMaster on a standalone player is a no-op (section 5.3)
- `C-42-setmaster-bare-master` -- a bare /SetMaster on a master is a no-op and does not dissolve its group (section 5.3)
- `C-43-setmaster-bare-slave` -- a bare /SetMaster on a slave leaves the group (section 5.3)
- `C-44-setmaster-master-param` -- /SetMaster?master= joins that player's group (section 5.3)
- `C-45-setmaster-slave-param` -- /SetMaster ignores a slave= parameter (section 5.3)
- `C-47-setmaster-noport` -- port is optional on /SetMaster?master= (section 5.3)
- `C-50-setmaster-reparent` -- a slave can be reparented onto another master with ?master= (section 5.1)
- `C-51-setmaster-nested` -- a bare /SetMaster on a nested master detaches it cleanly (section 14)
- `C-54-setmaster-one-sided` -- a group formed by /SetMaster?master= is one-sided: the master does not list the joiner as a <slave> (section 5.3)

---

## Capture fidelity

0 captured bodies are **byte-for-byte what the device sent** -- redaction changed nothing in them, so they are usable as parser fixtures and as evidence about byte lengths without qualification.

12 were modified. For each, `body_bytes_wire` in `MANIFEST.json` gives the original length and `redaction_edits` the number of substitutions, so a length-sensitive claim can still be checked. The digest recorded in the bundle is of the **redacted** body; because redaction is deterministic and stable within a run, two redacted digests still compare correctly against each other. Digests of the original bodies are in the do-not-share key file only, because for a templated body they would recover the redacted values by brute force.

| probe | endpoint | wire bytes | written bytes | edits |
|---|---|---|---|---|
| `001-state_setmaster` | `/SetMaster` | 412 | 408 | 2 |
| `005-state_setmaster` | `/SetMaster` | 542 | 534 | 3 |
| `011-state_setmaster` | `/SetMaster` | 395 | 391 | 2 |
| `014-state_setmaster` | `/SetMaster` | 395 | 391 | 2 |
| `021-state_setmaster` | `/SetMaster` | 395 | 391 | 2 |
| `027-state_setmaster` | `/SetMaster` | 395 | 391 | 2 |
| `033-state_setmaster` | `/SetMaster` | 395 | 391 | 2 |
| `040-state_setmaster` | `/SetMaster` | 437 | 429 | 3 |
| `047-state_setmaster` | `/SetMaster` | 542 | 534 | 3 |
| `055-state_setmaster` | `/SetMaster` | 542 | 534 | 3 |
| `060-state_setmaster` | `/SetMaster` | 412 | 408 | 2 |
| `065-state_setmaster` | `/SetMaster` | 529 | 521 | 3 |
